...
#define BUILD_TIMESTAMP "2016Äê 10ÔÂ 12ÈÕ"
...
mqttConnectionSuccess g_success  = NULL;
...
void MQTTClient_emptyMessageQueue(Clients* client)
{
	...
	if (client->messageQueue->count > 0)
	{
		...
		while (ListNextElement(client->messageQueue, &current))
		{
			...
			MQTT_SAFE_FREE(qe->topicName);
			MQTT_SAFE_FREE(qe->msg->payload);
			MQTT_SAFE_FREE(qe->msg);
		}
...
void MQTTClient_destroy(MQTTClient* handle)
{
	...
	if (m->c)
	{
		...
		MQTT_SAFE_FREE(saved_clientid);

	}
	if (m->serverURI)
	{
		MQTT_SAFE_FREE(m->serverURI);
	}
...
{
	FUNC_ENTRY;
	MQTT_SAFE_FREE((*message)->payload);
	MQTT_SAFE_FREE(*message);
	*message = NULL;
	FUNC_EXIT;
}
...
void MQTTClient_Suspend()
{
	tostop = 1;
}
void MQTTClient_Resume()
{
	tostop = 0;
}
void MQTTClient_stop()
	...
	START_TIME_TYPE start, long millisecsTimeout)
{
	...
	if (m->c->connect_state == 1) /* TCP connect started - wait for completion */
	{
		long timeout = 0;
		Thread_unlock_mutex(mqttclient_mutex);
		timeout =  millisecsTimeout - MQTTClient_elapsed(start);
		if(timeout < 0)
		{
			timeout = 0;
		}
		MQTTClient_waitfor(handle, CONNECT, &rc, timeout);
		Thread_lock_mutex(mqttclient_mutex);
		if (rc != 0)
		{
			rc = SOCKET_ERROR;
...
exit:
	...
	{
		Thread_unlock_mutex(mqttclient_mutex);
		MQTTClient_disconnect1(handle, 0, 0, 1); /* not "internal" because we don't want to call connection lost */
		if (MQTTVersion == 4)
		{
			Thread_lock_mutex(mqttclient_mutex);
		}
...
int MQTTClient_connectURI(MQTTClient handle, MQTTClient_connectOptions* options, const char* serverURI)
{
	...
	if (m->c->will)
	{
		MQTT_SAFE_FREE(m->c->will->msg);
		MQTT_SAFE_FREE(m->c->will->topic);
		MQTT_SAFE_FREE(m->c->will);
		m->c->will = NULL;
	}
	...
	if (m->c->sslopts)
	{
		if (m->c->sslopts->trustStore)
			MQTT_SAFE_FREE((void*)m->c->sslopts->trustStore);
		if (m->c->sslopts->keyStore)
			MQTT_SAFE_FREE((void*)m->c->sslopts->keyStore);
		if (m->c->sslopts->privateKey)
			MQTT_SAFE_FREE((void*)m->c->sslopts->privateKey);
		if (m->c->sslopts->privateKeyPassword)
			MQTT_SAFE_FREE((void*)m->c->sslopts->privateKeyPassword);
		if (m->c->sslopts->enabledCipherSuites)
			MQTT_SAFE_FREE((void*)m->c->sslopts->enabledCipherSuites);
		MQTT_SAFE_FREE(m->c->sslopts);
		m->c->sslopts = NULL;
	}
...
MQTTPacket* MQTTClient_waitfor(MQTTClient handle, int packet_type, int* rc, long timeout)
{
	...
	else
	{
		...
		while (!tostop)
		{
			...
			//printf("MQTTClient_waitfor MQTTClient_cycle pack=%d handle=%d rc=%d sock=%d\n",pack,handles,*rc,sock);
			if (m->c != NULL && sock == m->c->net.socket)
			{
				if (*rc == SOCKET_ERROR)
					break;
