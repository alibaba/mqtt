...
thread_type Thread_start(thread_fn fn, void* parameter)
{
	...
	if(WaitForSingleObject(thread, 10ul) != (DWORD)WAIT_TIMEOUT)
	{
		CloseHandle(thread);
	}
#else
...
int Thread_join(thread_type thread)
{
FUNC_ENTRY;
#if defined(WIN32) || defined(WIN64)
...
int Thread_isAlive(thread_type thread)
{
	FUNC_ENTRY;
#if defined(WIN32) || defined(WIN64)
	do{
...
// 		if (lockCount > 1)
// 		{
// 			//printf("Thread_lock_mutex error %d\n",g_count);
// 		}
		//printf("lock:%d\n",mutex);
#else
...
// 		if (lockCount < 0)
// 		{
// 			//printf("Thread_unlock_mutex error %d\n",g_count);
// 		}
		/* don't add entry/exit trace points as the stack log uses mutexes - recursion beckons */
#if defined(WIN32) || defined(WIN64)
		//printf("Unlock:%d\n",mutex);
