...
int Log_initialize(Log_nameValue* info)
{
	...
	if (info)
	{
		while (info->name)
		{
			snprintf(msg_buf, sizeof(msg_buf)-1, "%s: %s", info->name, info->value);
...
static char* Log_formatTraceEntry(traceEntry* cur_entry)
{
...
#if WIN32
	memset(msg_buf,' ',22);
    sprintf(msg_buf,"%d ",GetTickCount());
	buf_pos = strlen(msg_buf);
#else
#if defined(GETTIMEOFDAY)
    ...
    buf_pos = strlen(msg_buf);
#if defined(GETTIMEOFDAY)
	sprintf(&msg_buf[buf_pos], ".%.3lu ", cur_entry->ts.tv_usec / 1000L);
#else
	sprintf(&msg_buf[buf_pos], ".%.3hu ", cur_entry->ts.millitm);
#endif
	...
	else
	{
		...
		if (cur_entry->has_rc == 1)
			snprintf(&msg_buf[buf_pos], sizeof(msg_buf)-1-buf_pos, format, cur_entry->thread_id,
...
static void Log_posttrace(int log_level, traceEntry* cur_entry)
{
	if (((trace_output_level == -1) ? log_level >= trace_settings.trace_level : log_level >= trace_output_level))
	{
		...
		if (trace_destination || trace_callback)
            msg = Log_formatTraceEntry(cur_entry);
